...
func TestStoresPlaintextProviderSecret(t *testing.T) {
	cases := []struct {
		name   string
		cipher SecretCipher
		secret string
		...
		{"no cipher, secret present", nil, "s3cret", true},
		{"cipher configured", noopCipher{}, "s3cret", false},
		{"no secret supplied", nil, "", false},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := storesPlaintextProviderSecret(tc.cipher, tc.secret); got != tc.want {
				t.Fatalf("storesPlaintextProviderSecret(%v, secret) = %v, want %v",
					tc.cipher != nil, got, tc.want)
			}
		})
	}
}

func TestWarnPlaintextProviderSecretFires(t *testing.T) {
	log, buf := captureLogs()
	s := &Store{log: log}

	s.warnPlaintextProviderSecret("s3cret", "github", "seed")
	...
	if msg, _ := rec["msg"].(string); msg != "provider secret stored without at-rest encryption" {
		t.Errorf("msg = %q, want the provider plaintext-secret warning", msg)
	}
	if p, _ := rec["provider"].(string); p != "github" {
		t.Errorf("provider = %q, want github", p)
	}
	if act, _ := rec["action"].(string); act != "seed" {
		t.Errorf("action = %q, want seed", act)
	}
	// The secret must never reach the log.
	if strings.Contains(buf.String(), "s3cret") {
		t.Fatal("the provider secret leaked into the log record")
	}
}

func TestWarnPlaintextProviderSecretStaysQuiet(t *testing.T) {
	cases := []struct {
		name   string
		cipher SecretCipher
		secret string
	}{
		{"cipher configured", noopCipher{}, "s3cret"},
		{"no secret", nil, ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			log, buf := captureLogs()
			s := &Store{log: log, secretCipher: tc.cipher}
			s.warnPlaintextProviderSecret(tc.secret, "github", "rotate")
			if n := len(records(t, buf)); n != 0 {
				t.Fatalf("expected no log records, got %d: %s", n, buf.String())
			}
		})
...
// A store with no logger attached must not panic: WithLogger is optional.
func TestWarnPlaintextProviderSecretNilLogger(t *testing.T) {
	s := &Store{}
	s.warnPlaintextProviderSecret("s3cret", "github", "seed")
}

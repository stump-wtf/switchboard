...
func TestVisibilityLeaseAndHeartbeat(t *testing.T) {
	...
	td, _, err := s.CreateTodo(ctx, CreateTodoParams{Queue: "q", Title: "t", IdempotencyKey: "lease1"})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	claimed, err := s.ClaimTodo(ctx, td.ID, "owner-1", time.Hour)
	if err != nil {
	...
	if _, err := s.ClaimNext(ctx, []string{"q"}, "owner-2", time.Minute); !errors.Is(err, ErrNotFound) {
	...
	if _, err := s.HeartbeatTodo(ctx, td.ID, "owner-2", 2*time.Hour); !errors.Is(err, ErrConflict) {
		t.Fatalf("non-owner heartbeat should conflict, got %v", err)
	}
	// Owner heartbeat extends the lease without consuming an attempt.
	hb, err := s.HeartbeatTodo(ctx, td.ID, "owner-1", 3*time.Hour)
	if err != nil {
	...
	if _, err := s.CompleteTodo(ctx, td.ID, "owner-2", nil); !errors.Is(err, ErrConflict) {
		t.Fatalf("non-owner complete should conflict, got %v", err)
	}
	if _, err := s.FailTodo(ctx, td.ID, "owner-2", nil); !errors.Is(err, ErrConflict) {
		t.Fatalf("non-owner fail should conflict, got %v", err)
	}

	// Heartbeat on an absent todo is ErrNotFound.
	if _, err := s.HeartbeatTodo(ctx, "td_missing", "owner-1", time.Hour); !errors.Is(err, ErrNotFound) {
...
func TestReaperRequeuesAndDeadLetters(t *testing.T) {
	...
	a, _, err := s.CreateTodo(ctx, CreateTodoParams{Queue: "q", Title: "a", IdempotencyKey: "reap-a"})
	if err != nil {
		t.Fatalf("create a: %v", err)
	}
	if _, err := s.ClaimTodo(ctx, a.ID, "w", time.Hour); err != nil {
		t.Fatalf("claim a: %v", err)
	}
	// At the cap: an expired lease is dead-lettered to failed.
	b, _, err := s.CreateTodo(ctx, CreateTodoParams{Queue: "q", Title: "b", IdempotencyKey: "reap-b"})
	if err != nil {
		t.Fatalf("create b: %v", err)
	}
	if _, err := s.ClaimTodo(ctx, b.ID, "w", time.Hour); err != nil {

...
related: [ADR-0012, ADR-0013, ADR-0022, ADR-0025, ADR-0029]
---
...
* **Multi-tenancy.** A trust list and a quarantine belong to the webhook's owner scope, like
  everything else a user creates (ADR-0022, and the Teams ADR-0038 in progress). The instance
...
4. Let everything else quarantine.
...
* Good, because rule packs (ADR-0036, in progress) and the one-shot trigger path get a gate they
  can rely on.
...
* Bad, because logins are mutable. A trusted user who renames frees their old login for someone
  else. This is carried as an open question: numeric forge ids as an alternative entry form.
...
* Parallel records, cited by number until they merge:

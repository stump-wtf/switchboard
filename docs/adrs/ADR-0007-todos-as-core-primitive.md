...
**Implementation status (partial):** the todo lifecycle and the drain verbs below (`list_todos`, `claim`, `complete`, `fail`, plus the lease heartbeat) are implemented and exposed as MCP tools. **`create_for` is not.** A store backend for it exists (`internal/store/friends.go`), but no MCP tool registers the verb, so no vended endpoint is ever given it and one agent cannot hand a todo to another today. The decision recorded here — the todo as the core primitive — is accepted and unaffected by that gap; only the cross-agent creation verb remains unexposed. See the [Friending guide](../guides/04-friending.md) for what works instead.

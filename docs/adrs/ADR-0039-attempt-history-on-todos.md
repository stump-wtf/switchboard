...
related: [ADR-0013, ADR-0028, ADR-0029, ADR-0027]
---
...
* **Every committed claim opens exactly one attempt**, in the same transaction as the `todos` update. That covers `claim`, `claim_next`, a lease takeover, and a human claiming from the Board. It is the same event queue admission control (ADR-0035 / SPEC-0030, in flight) charges one unit for, so the two count identically.
...
* **Records in flight, cited by number until they merge:** SPEC-0034 (this ADR's spec, same PR), ADR-0034 / SPEC-0029 (notification sinks), ADR-0035 / SPEC-0030 (admission control), ADR-0038 / SPEC-0033 (teams and tenancy), SPEC-0024 (notify hooks), Harness ADR-0025 / SPEC-0019 (relay attempts), and Cairn ADR-0027 / SPEC-0021 (receipts).

...
Governing spec: SPEC-0025. Amended: SPEC-0011 (the `ring_id` meta key only). The per-session
unheard mark is new in SPEC-0025, alongside the write-failure warning, which lives in code.
...
-- No unheard_at filter: the claim-ack UPDATE must still find unheard rows to record a late ack
-- (REQ-4 "A late claim") without a sequential scan inside the lease transaction.
...
- **Should the board offer a "run self-test" button on the endpoint card?** Resolved (design review 2026-09-22): yes, as proposed. It
  calls the same REQ-10 route, and it is the most discoverable proof gate for a new user.
- **Should an `unheard` session also be exposed to the agent itself?** Resolved (design review 2026-09-22): not in v1. It is
  reconsidered once the self verbs from SPEC-0022 have landed.

...
related: [SPEC-0022, SPEC-0024, SPEC-0027, SPEC-0033]
---
...
per-session unheard mark (REQ-5) alongside it. [SPEC-0024](../notify-hooks/spec.md) (notify hooks)
never fires a hook for a synthetic todo (REQ-9). It relies on the **self verb** mechanism that [SPEC-0022](../endpoint-presence/spec.md) REQ "Self
...
endpoint. The instance operator role MUST NOT widen what either route can reach. The board's endpoint
card MUST offer a "Run self-test" control that starts a test through the same route and shows the
REQ-8 report.
...
The board's "Run self-test" control (REQ-10) MUST be a POST with the existing CSRF token.

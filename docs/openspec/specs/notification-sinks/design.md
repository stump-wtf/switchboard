...
* **The connection vault** from SPEC-0028 (`outbound_connections`) holds the credentials.
...
- **Should a sink subscribe to unheard doorbells in real time?** Resolved (design review 2026-09-22): no for v1, as proposed. Unheard
  doorbells reach humans through the digest until SPEC-0025's thresholds are measured.
- **Should owners be allowed a budget above the operator default?** Resolved (design review 2026-09-22): yes, as proposed, within an
  operator ceiling (REQ-11).
- **Gotify markdown.** Resolved (design review 2026-09-22): left out of v1, so every sink receives the same text.
- **Durable outbox or best-effort delivery?** Resolved (design review 2026-09-22): the transactional outbox (REQ-10).
- **Agent paging.** Resolved (design review 2026-09-22): `notify_owner` is the only agent paging verb. It is fire-and-forget,
  owner-granted and off by default. There is no acknowledgement, assignee or ticket state.

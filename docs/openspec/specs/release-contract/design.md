...
`v0.2.0` is 17 commits behind `main`. The commits in between include #291, which removed the
...
- **Should Harness and Cairn adopt the same contract?** This ADR is written to be portable. Those
  repositories would each record their own adoption.

...
- Should the grace window after `complete` be per owner rather than instance-wide? Proposed:
  instance-wide default, per-connection override later if asked.
- Should `complete` and `fail` take an optional `reply` field that posts atomically with the state
  change? Proposed: no for v1; two calls keep each outcome independently recorded, and Harness can
  make both on the worker's behalf.
- GitHub Apps: an installation token expires hourly. v1 accepts personal or fine-grained tokens; an App
  credential kind (private key plus installation id, minting tokens per call) is a follow-up.

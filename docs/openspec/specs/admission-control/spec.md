...
related: [SPEC-0022, SPEC-0024, SPEC-0029, SPEC-0033, SPEC-0034]
---
...
Companion records, accepted together on 2026-09-22 and linked as front-matter edges: queue identity and team roles are ADR-0038 /
...
lookup that finds no policy. A request that is malformed, has an unknown zone, sets `weekday` on a
period other than `week` (or omits it on `week`), or sets neither limit MUST be refused with `invalid_argument`, naming
...
(the count of pending todos), `next_eligible_at` and `exact`, as REQ-6 defines it. `empty: true`
without `deferred` MUST keep meaning

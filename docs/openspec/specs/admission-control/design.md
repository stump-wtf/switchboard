...
Queue identity follows the Teams contract (ADR-0038 / SPEC-0033): a queue is
...
* **Are retries and lease takeovers charged?** Resolved (design review 2026-09-22) (Joe): yes. Every committed claim is charged,
  retries and takeovers included, and nothing refunds a charge (REQ-3).
* **Should a team queue policy cap each draining endpoint's share?** Resolved (design review 2026-09-22): not in this program. It is
  reconsidered once team queues (SPEC-0033) exist.
* **Should `admission_status` be a self verb that needs no grant?** Resolved (design review 2026-09-22): no. It rides on the drain
  grant, as designed.
* **Should the queue digest warn when a budget is mostly spent early?** Resolved (design review 2026-09-22): not in v1. The digest
  reports deferred work (SPEC-0029), and an early-spend warning is a candidate for a later digest
  change, not for this spec.

...
On a `github`, `gitea` or `cairn` webhook, the work order also carries `author_trusted`: `false` means a trusted sender moved work whose text an untrusted author wrote. Treat that text with extra suspicion. It is `null` under `allow_all`, where there is no per-actor verdict, and absent only on a source with no trust gate. A worker treats `null` or absent as not trusted.

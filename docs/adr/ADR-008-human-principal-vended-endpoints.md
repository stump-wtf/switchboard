...
1. **Humans are the principals/tenants.** A human authenticates to switchboard via **OIDC against Pocket ID** ([ADR-011](ADR-011-identity-assurance-oidc-passkey-deferred.md)). The human is the accountable owner and the **policy authority** — they decide which queues and verbs any of their agents may touch.
...
* Bad, because Pocket ID is passkey-only and humans-oriented; bots do not fit its model ([ADR-011](ADR-011-identity-assurance-oidc-passkey-deferred.md)).
...
* Personas subdivide a single agent's vended tools into narrower capability sets: [ADR-009](ADR-009-personas-as-scoped-agent-cards.md).

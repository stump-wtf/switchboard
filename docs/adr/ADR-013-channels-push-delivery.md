...
* **Symmetry with the human UI.** Humans are pushed via the web UI's SSE ([ADR-001](ADR-001-web-stack-starlette-htmx-pico.md)); agents/harnesses are pushed via Channels. **Two audiences, two push transports, one durable ledger** ([ADR-002](ADR-002-sqlite-persistence-and-retention.md)).

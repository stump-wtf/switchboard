...
* **Least privilege.** The service's policy grants read on only its own KV path (`secret/webhook-mcp/*`), nothing else.
...
Chosen option: **"(D) OpenBao AppRole."** The service authenticates to `vault.stump.rocks` using an **AppRole** (`role_id` + `secret_id`), receives a short-lived Vault token, and reads its secrets from a KV v2 mount under `secret/webhook-mcp/*`. This is the only option that gives machine identity, a scoped policy, rotation/revocation independent of humans, and no committed secret material — and it matches the established StumpCloud pattern.
...
4. **Read:** `GET secret/data/webhook-mcp/...` for each needed secret (KV v2). Secrets are cached in memory keyed by provider.
...
### Secret layout (KV v2 under `secret/webhook-mcp/`)

| Path | Keys | Consumer |
|------|------|----------|
| `secret/webhook-mcp/providers/github` | `hmac_secret` | GitHub signature verification ([ADR-003](ADR-003-per-provider-ingestion-and-trust-model.md)) |
| `secret/webhook-mcp/providers/stripe` | `signing_secret` | Stripe signature verification |
| `secret/webhook-mcp/providers/slack` | `signing_secret` | Slack signature verification |
| `secret/webhook-mcp/generic/<name>` | `token` | Generic endpoint shared-token bozo-filter |
| `secret/webhook-mcp/redis` | `url` (may embed password), optional `ca_cert` | Redis consumer connection |

The Vault **policy** for the AppRole grants `read` on `secret/data/webhook-mcp/*` and `secret/metadata/webhook-mcp/*` and nothing else.
...
* Good, because least-privilege policy confines the blast radius of a compromised token to `secret/webhook-mcp/*`.
...
* A startup integration test (against a dev/test Vault or a mock) asserts AppRole login → read of `secret/webhook-mcp/providers/github` succeeds and populates the in-memory secret cache.
...
* The AppRole policy is documented (in `stumpcloud/ansible` provisioning) as read-only on `secret/webhook-mcp/*`.
...
sequenceDiagram
    ...
    participant App as webhook-mcp
    ...
    App->>Bao: GET secret/data/webhook-mcp/providers/{github,stripe,slack}
    App->>Bao: GET secret/data/webhook-mcp/redis

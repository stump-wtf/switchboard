...
* **Secrets stay with switchboard.** The signing secret for a created webhook is minted and held by switchboard in OpenBao ([ADR-004](ADR-004-secrets-management-openbao-approle.md)); the agent gets only the URL to hand to the producer, never the secret material to store.
...
* **Switchboard mints the signing secret** for each created webhook and stores it in OpenBao under the agent's path ([ADR-004](ADR-004-secrets-management-openbao-approle.md) layout, extended). The **agent never sees or stores the secret.**
...
* Good, because secrets never leave switchboard/OpenBao — the agent handles URLs, not signing keys ([ADR-004](ADR-004-secrets-management-openbao-approle.md)).
...
* A test asserts the created webhook's signing secret is stored in OpenBao and is **never** returned to the agent; `create`/`rotate` return only the URL.
...
flowchart TB
  ...
  gate -- yes --> mk[switchboard mints secret → OpenBao,<br/>owns verification + idempotency]

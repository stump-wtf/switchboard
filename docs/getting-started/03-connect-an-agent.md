...
so copying the repository's `skills/` contents there works too. **Don't symlink instead**: Crush
resolves symlinks back out of the scanned directory, so a linked skill loads but its file reads sit
outside the skill path and get denied. Either way, point Crush at the **directory of skills**, not
at one skill's folder inside it.
